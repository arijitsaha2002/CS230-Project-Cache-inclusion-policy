...
class CACHE : public MEMORY {
  public:
    ...
    // ----------------------------------------------------------------------------
    uint32_t fifo_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    void fifo_update(uint32_t set, uint32_t way);
    uint32_t random_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    u_int32_t lfu_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type);
    void lfu_update(uint32_t set, uint32_t way);
    int issue_prefetch(t_addr lineaddr, int offset, u_int32_t prefetch_metadata, u_int64_t ip);
};
