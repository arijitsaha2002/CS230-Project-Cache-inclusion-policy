...
uint32_t CACHE::random_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type){
    ...
    srand(current_core_cycle[0]);
    way = rand()%NUM_WAY;

    return way;
}

u_int32_t CACHE::lfu_victim(uint32_t cpu, uint64_t instr_id, uint32_t set, const BLOCK *current_set, uint64_t ip, uint64_t full_addr, uint32_t type)
{
    uint32_t way = 0;
    ...
    // LRU victim
    u_int64_t correct_way = way;
    long long min_cnt = LONG_LONG_MAX;
    if (way == NUM_WAY) {
        for (way=0; way<NUM_WAY; way++) {
            if (block[set][way].lfu_cnt < min_cnt) {
