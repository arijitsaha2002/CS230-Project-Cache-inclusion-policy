    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (cache_type == IS_LLC) {
            ...
            if (block[set][way].valid) {
                uint64_t removingAddr = block[set][way].address;
                int numCPU = NUM_CPUS;
                while (numCPU --) {
                    if (ooo_cpu[numCPU].L2C.invalidate_entry(removingAddr) != -1) {
                        ooo_cpu[numCPU].L1D.invalidate_entry(removingAddr);
                        ooo_cpu[numCPU].L1I.invalidate_entry(removingAddr);
                    }
                }
        ...
        if (do_fill) {
            int do_fill_updated = 1;

#ifdef EXCLUSIVE
            // cout << 1 << endl;

            do_fill_updated = (fill_level == MSHR.entry[mshr_index].fill_level);
            ...
            if (do_fill_updated) {
                ...
                if (!block[set][way].dirty) {
                    ...
                    if (lower_level and block[set][way].address != 0) {
                        if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                ...
                if (mshr_index == -2)
                {
                    ...
                    if (cache_type == IS_LLC)
                    {
                        ...
                        else
                        {
                            ...
                            // is changed for exclusive

                            if (lower_level && RQ.entry[index].returned != COMPLETED )
                            {
